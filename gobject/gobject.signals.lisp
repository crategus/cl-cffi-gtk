...
(defun query-signal-info (signal-id)
  (with-foreign-object (q 'g-signal-query)
    ...
    (let ((param-types
           (iter (with param-types = (foreign-slot-value q
                                                         'g-signal-query
                                                         :param-types))
                 (for i from 0 below (foreign-slot-value q
                                                         'g-signal-query
                                                         :n-params))
      ...
      (make-signal-info :id signal-id
                        :name
                        (foreign-slot-value q 'g-signal-query :signal-name)
                        :owner-type
                        (foreign-slot-value q 'g-signal-query :owner-type)
                        :flags
                        (foreign-slot-value q 'g-signal-query :signal-flags)
                        :return-type
                        (foreign-slot-value q 'g-signal-query :return-type)
...
;;; 
;;;  1 g_signal_new ("size_request",
;;;  2   G_TYPE_FROM_CLASS (gobject_class),
;;;  3      G_SIGNAL_RUN_FIRST,
;;;  4      G_STRUCT_OFFSET (GtkWidgetClass, size_request),
;;;  5      NULL, NULL,
;;;  6      _gtk_marshal_VOID__BOXED,
;;;  7      G_TYPE_NONE, 1,
;;;  8      GTK_TYPE_REQUISITION | G_SIGNAL_TYPE_STATIC_SCOPE);
;;; ----------------------------------------------------------------------------
...
;;; flags, and the criteria values are passed as arguments. The match mask has
...
;;; one of the G_SIGNAL_MATCH_CLOSURE, G_SIGNAL_MATCH_FUNC or
;;; G_SIGNAL_MATCH_DATA match flags is required for successful matches. If no
;;; handlers were found, 0 is returned, the number of blocked handlers
;;; otherwise.
;;; 
;;; instance :
