...
(define-test gdk-screen
  (let* ((screen (gdk-screen-get-default))
    ...
    (let ((rect (gdk-screen-get-monitor-geometry screen n)))
      (assert-eql 0 (rectangle-x rect))
      (assert-eql 0 (rectangle-y rect))
      (assert-eql (%gdk-screen-width)  (rectangle-width rect))
      (assert-eql (%gdk-screen-height) (rectangle-height rect)))
