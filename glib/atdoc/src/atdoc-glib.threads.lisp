...
(setf (gethash 'g-thread-error atdoc:*type-name-alias*) "Enum")
(setf (documentation 'g-thread-error 'type)
 "@version{2012-12-23}
  @short{Possible errors of thread related functions.}
  @begin{pre}
(defcenum g-thread-error
  :g-thread-error-again)
  @end{pre}
  @begin{table}
    @entry[:g-thread-error-again]{a thread couldn't be created due to resource
      shortage. Try again later.}
  @end{table}")

;;; ----------------------------------------------------------------------------

(setf (gethash 'g-thread atdoc:*type-name-alias*) "CStruct")
(setf (documentation 'g-thread 'type)
 "@version{2012-12-23}
  @short{The @sym{g-thread} struct represents a running thread.}
  This struct is returned by @fun{g-thread-new} or @code{g_thread_try_new()}.
  You can obtain the @sym{g-thread} struct representing the current thead by
  calling @fun{g-thread-self}.

  @sym{g-thread} is refcounted, see @code{g_thread_ref()} and
  @code{g_thread_unref()}. The thread represented by it holds a reference while
  it is running, and @fun{g-thread-join} consumes the reference that it is
  given, so it is normally not necessary to manage @sym{g-thread} references
  explicitly.

  The structure is opaque -- none of its fields may be directly accessed.
  @see-function{g-thread-new}
  @see-function{g-thread-self}
  @see-function{g-thread-join}")
...
(setf (documentation 'g-thread-new 'function)
 "@version{2012-12-23}
  @argument[name]{a name for the new thread}
  @argument[func]{a function to execute in the new thread}
  @argument[data]{an argument to supply to the new thread}
  @return{the new @type{g-thread}}
  @short{This function creates a new thread.}
  
  The new thread starts by invoking @arg{func} with the argument @arg{data}. The
  thread will run until @arg{func} returns or until @code{g_thread_exit ()} is
  called from the new thread. The return value of @arg{func} becomes the return
  value of the thread, which can be obtained with @fun{g-thread-join}.

  The @arg{name} can be useful for discriminating threads in a debugger. Some
  systems restrict the length of @arg{name} to 16 bytes.

  If the thread can not be created the program aborts. See
  @code{g_thread_try_new ()} if you want to attempt to deal with failures.

  To free the struct returned by this function, use @code{g_thread_unref ()}.
  Note that @fun{g-thread-join} implicitly unrefs the @type{g-thread} as well.
...
(setf (documentation 'g-thread-join 'function)
 "@version{2012-12-23}
  @argument[thread]{a @type{g-thread}}
  @return{the return value of the thread}
  @short{Waits until thread finishes, i. e. the function @arg{func}, as given to
    @fun{g-thread-new}, returns or @code{g_thread_exit ()} is called.}

  If thread has already terminated, then @sym{g-thread-join} returns
  immediately.

  Any thread can wait for any other thread by calling @sym{g-thread-join}, not
  just its 'creator'. Calling @sym{g-thread-join} from multiple threads for the
  same thread leads to undefined behaviour.

  The value returned by @arg{func} or given to @code{g_thread_exit ()} is
  returned by this function.

  @sym{g-thread-join} consumes the reference to the passed-in thread. This will
  usually cause the @type{g-thread} struct and associated resources to be freed.
  Use @code{g_thread_ref ()} to obtain an extra reference if you want to keep
  the @type{g-thread} alive beyond the @sym{g-thread-join} call.
  @see-function{g-thread-new}")
...
(setf (documentation 'g-thread-self 'function)
 "@version{2012-12-23}
  @return{the @type{g-thread} representing the current thread}
  @begin{short}
    This functions returns the @type{g-thread} corresponding to the current
    thread.
  @end{short}
  Note that this function does not increase the reference count of the returned
  struct.

  This function will return a @type{g-thread} even for threads that were not
...
(setf (documentation 'g-mutex 'type)
 "@begin{short}
    The @sym{g-mutex} struct is an opaque data structure to represent a mutex
    (mutual exclusion).
  @end{short}
  It can be used to protect data against shared access. Take for example the
  following function:
...
(setf (documentation 'g-cond 'type)
 "@begin{short}
    The @sym{g-cond} struct is an opaque data structure that represents a
    condition.
  @end{short}
  Threads can block on a @sym{g-cond} if they find a certain condition to be
  false. If other threads change the state of this condition they signal the
  @sym{g-cond}, and that causes the waiting threads to be woken up.
  ...
  Example 6.  Using @sym{g-cond} to block a thread until a condition is
  satisfied
  @begin{pre}
...
;;; --- End of file atdoc-glib.threads.lisp ------------------------------------
