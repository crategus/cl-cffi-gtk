...
;;; 
;;; GtkStyleContext has built-in support for state change transitions. Note
;;; that these animations respect the "gtk-enable-animations" setting.
;;; 
;;; For simple widgets where state changes affect the whole widget area,
;;; calling gtk_style_context_notify_state_change() with a NULL region is
;;; sufficient to trigger the transition animation. And GTK+ already does that
;;; when gtk_widget_set_state() or gtk_widget_set_state_flags() are called.
;;; 
...
;;; Theme engines can also use style classes to vary their rendering. GTK+ has
;;; a number of predefined style classes:
;;;
;;; GTK_STYLE_CLASS_CELL,
;;; GTK_STYLE_CLASS_ENTRY,
;;; GTK_STYLE_CLASS_BUTTON,
...
;;; GtkStyleProvider interface. This way theming engines may still attempt to
...
;;; implementation, as GtkCssProvider uses gtk_theming_engine_load() which
;;; loads the theming engine module from the standard paths.
;;;
;;; ----------------------------------------------------------------------------
;;;
;;; Property Details
...
;;; 
;;; This is used when drawing an outline around a potential drop target
;;; during DND.
;;; ----------------------------------------------------------------------------
...
;;; 
;;; A CSS class for an area displaying a question to the user, such as those
;;; in infobars.
;;; 
;;; This is used by GtkInfoBar.
;;; ----------------------------------------------------------------------------
...
;;; 
;;; priority :
;;;     the priority of the style provider. The lower it is, the earlier it
;;;     will be used in the style construction. Typically this will be in the
;;;     range between GTK_STYLE_PROVIDER_PRIORITY_FALLBACK and
...
;;; gtk_style_context_add_provider() takes precedence over another added
...
;;; const and will remain valid until the "changed" signal happens.
;;; 
;;; context :
;;;     a GtkStyleContext
;;; 
...
;;; to being set. This means transition animation will run from 0 to 1 when
;;; state is being set and from 1 to 0 when it's being unset.
;;; 
;;; context :
;;;     a GtkStyleContext
;;; 
...
;;; with a GtkStyleContext obtained through gtk_widget_get_style_context(),
;;; this is normally done for you in all circumstances you would expect all
;;; widget to be stopped, so this should be only used in complex widgets with
;;; different animatable regions.
;;; 
;;; context :
;;;     a GtkStyleContext
;;; 
...
;;; 
;;; Returns :
;;;     a GList of strings with the currently defined regions. The contents
;;;     of the list are owned by GTK+, but you must free the list itself with
;;;     g_list_free() when you are done with it.
;;; 
;;; Since 3.0
;;; ----------------------------------------------------------------------------
...
;;; 
;;; Renders a extension (as in a GtkNotebook tab) in the rectangle defined by
;;; x, y, width, height. The side where the extension connects to is defined
;;; by gap_side.
;;; 
;;; context :
;;;     a GtkStyleContext
...
;;; GTK_POS_TOP and GTK_POS_BOTTOM gap sides, and Y coordinates for
...
;;;                         gdouble height);
;;; 
;;; Renders a handle (as in GtkHandleBox, GtkPaned and GtkWindow's resize
...
;;;                                     GtkIconSize size);
;;; 
;;; Renders the icon specified by source at the given size, returning the
;;; result in a pixbuf.
;;; 
;;; context :
;;;     a GtkStyleContext
