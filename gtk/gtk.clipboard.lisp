...
(setf (documentation 'gtk-clipboard 'type)
 "@version{2021-10-6}
  ...
  Conversion to and from strings can be done with the @fun{gdk-atom-intern} and
  @fun{gdk-atom-name} functions. The default clipboard corresponds to the
...
(setf (gethash 'gtk-clipboard-targets-received-func atdoc:*symbol-name-alias*)
 ...
 lambda (clipboard atoms n-atoms)
  ...
  @begin[code]{table}
    @entry[clipboard]{A @class{gtk-clipboard} object.}
    @entry[atoms]{A pointer to a foreign C array of @symbol{gdk-atom} atoms.}
...
(defun gtk-clipboard-wait-for-rich-text (clipboard buffer)
 ...
 "@version{2021-10-6}
  ...
  @begin{return}
    ...
    data failed. @br{}
    @arg{format} -- a @symbol{gdk-atom} as a string with the format of the
      returned data @br{}
