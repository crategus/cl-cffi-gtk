...
(setf (documentation 'gtk-list-store 'type)
 "@version{2013-3-28}
  ...
  @subheading{Atomic Operations}
    ...
    \"row-inserted\" signal on GtkListStore. The row, however, is still empty,
    and any signal handler connecting to \"row-inserted\" on this particular
    store should be prepared for the situation that the row might be empty. This
    is especially important if you are wrapping the GtkListStore inside a
...
(defun gtk-list-store-new (&rest column-types)
 #+cl-cffi-gtk-documentation
 "@version{2013-3-28}
  @argument[n_columns]{number of columns in the list store}
  @argument[...]{all GType types for the columns, from first to last}
...
(defcfun ("gtk_list_store_set_column_types" gtk-list-store-set-column-types)
 ...
 "@version{2013-3-28}
  ...
  @argument[n_columns]{Number of columns for the list store}
  @argument[types]{An array length n of GTypes}
