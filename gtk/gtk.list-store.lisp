...
;;; supported.
;;; 
;;; As an example, gtk_tree_store_new
;;; (3, G_TYPE_INT, G_TYPE_STRING, GDK_TYPE_PIXBUF); will create a new
;;; GtkListStore with three columns, of type int, string and GdkPixbuf
;;; respectively.
;;; 
;;; n_columns :
...
(defmethod initialize-instance :after ((store gtk-list-store)
                                       &rest initargs
                                       &key (column-types
                                              nil
...
(defun gtk-list-store-value (list-store iter column)
  (gtk-tree-model-get-value list-store iter column))
  ...
  (let ((n (length values))
    ...
    (with-foreign-objects ((v-ar 'g-value n)
      ...
      (iter (for i from 0 below n)
            ...
            (set-g-value (mem-aref v-ar 'g-value i)
                         value
                         type
                         :zero-g-value t))
