...
(test g-enum-value
  ...
  (is (equal '(:value :name :nick) (foreign-slot-names '(:struct g-enum-value)))))

;;;   g-enum-class

(test g-enum-class
  (is (= 20 (foreign-type-size '(:struct g-enum-class))))
  (is (equal '(:type-class :minimum :maximum :n-values :values)
             (foreign-slot-names '(:struct g-enum-class)))))

;;;   g-flags-value

(test g-flags-value
  (is (= 12 (foreign-type-size '(:struct g-flags-value))))
  (is (equal '(:value :name :nick) (foreign-slot-names '(:struct g-flags-value)))))

;;;   g-flags-class

(test g-flags-class
  (is (= 16 (foreign-type-size '(:struct g-flags-class))))
  (is (equal '(:type-class :mask :n-values :values)
             (foreign-slot-names '(:struct g-flags-class)))))
